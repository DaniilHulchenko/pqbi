...
public class EngineCalculationService : IEngineCalculationService
{
    ...
    private IEnumerable<GraphParametersComponentDtoV3> GetAxisForBaseParameter(CustomParameterNodeCalculator node)
    {
        ...
        foreach (var baseParameter in node.BaseParameterComponents)
        {
            ...
            if (baseParameter.Axis is null)
            {
                ...
                baseParameter.SetRawData(new PQBIAxisDataEmpty(baseParameter.ComponentID, baseParameter.FeederId, baseParameter.BaseParameterName, PQZStatus.GENERAL_ERROR, baseParameter.DataUnitType));
            ...
            var points = baseParameter.Axis.DataTimeStamps.Select(x => new BasicValue(x.Point, x.DateTime, x.DataValueStatus.ToPqbiDataValueStatus())).ToArray();
            IEnumerable<IEnumerable<BasicValue>> data = null;
            ...
            if (baseParameter.Axis.PQZStatus == PQZStatus.OK)
            {
                if (node.AdvancedSettingsForTable?.IsExcludeFlaggedData == true)
                {
                    ...
                    nominalVal = SetNominalAndUnitType(normalizeEnum, nominalVal, baseParameter);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (normalizeEnum == NormalizeEnum.NO)
                        {
                            ...
                            {
                                GroupByFunctionInput groupByOperation = new GroupByFunctionInput(points, 0);
                                data = _engineCalculator.CalcGroupByAsync(groupByOperation);
                                baseParameterAxis = (List<AxisValue>)CalculateQuantityFunction(aggregationQuantity, node.StartDate.ToDateTimeOffsetInSeconds(), 0, data);
                            }
                            else
                            {
                                foreach (var dataTimeStamp in baseParameter.Axis.DataTimeStamps)
                                {
                                    baseParameterAxis.Add(new AxisValue { Value = dataTimeStamp.Point, TimeStempInSeconds = dataTimeStamp.DateTime.ToDateTimeOffsetInSeconds(), DataValueStatus = dataTimeStamp.DataValueStatus });
                                }
                            }
                        }
                        else
                        {
                            ...
                            if (node.IsSinglePointRes && baseParameter.Axis.DataTimeStamps.Count() > 1)
                            {
                                ...
                                baseParameterAxis = (List<AxisValue>)CalculateQuantityAndNormalizeFunction(aggregationQuantity, node.StartDate.ToDateTimeOffsetInSeconds(), 0, data, nominalVal);                                
                            }
                            else
                            {
                                foreach (var dataTimeStamp in baseParameter.Axis.DataTimeStamps)
                                {
                                    ...
                                    baseParameterAxis.Add(new AxisValue { Value = pointVal, TimeStempInSeconds = dataTimeStamp.DateTime.ToDateTimeOffsetInSeconds(), DataValueStatus = dataTimeStamp.DataValueStatus });
                                }
                            }
                        }
                    }
            ...
            if (node.IsGroupByTime)
            {
                ...
                {
                    foreach (var item in baseParameterAxis)
                    {
                        var tmp = BaseParameterPrepareBaseParameterResponse(baseParameter.ScadaParameterName, prmNameWithFeeder, baseParameter.ComponentID, baseParameter.FeederId, baseParameter.DataUnitType, node.CustomParameterType, [item], missings);
                        response.Add(tmp);
                    }
                }
                else
                {                   
                    foreach (var item in node.BarTimeRangeList)
                    {
    ...
    private IEnumerable<AxisValue> CalculateGroupsAndGetAxisValues(CustomParameterNodeCalculator node, string aggregationQuantity, NormalizeEnum normalizeEnum, double nominalVal, int resolutionInSeconds, IEnumerable<IEnumerable<BasicValue>> data, bool isExcludeFlagged)
    {
        IEnumerable<AxisValue> axisValCollection;
        List<BasicValue[]> dataGroupList = data.Select(group => group.ToArray()).ToList();
        dataGroupList = ParameterMatrix.FilterByFlaggedEvents(isExcludeFlagged, dataGroupList);
    ...
    private static double SetNominalAndUnitType(NormalizeEnum normalizeEnum, double nominalVal, BaseParameterComponent baseParameter)
    {
        if (normalizeEnum != NormalizeEnum.NO)
        {
    ...
    private IEnumerable<IEnumerable<AxisValue>> GetAxisForSingleParameter(CustomParameterNodeCalculator node)
    {
        List<IEnumerable<AxisValue>> response = new List<IEnumerable<AxisValue>>();
        ...
        using (var mainLogger = PqbiStopwatch.Anchor($"{nameof(GetAxisForSingleParameter)}"))
        {
            ...
            IEnumerable<IEnumerable<BasicValue>> data;
            if (node.IsGroupByTime)
            {
                data = GroupByTime.AggregateToBars(finalMatrix.AggregationCalculation, node.BarTimeRangeList);
            }
            else
            {
                if (node.IsWidgetResolutionAuto)
            ...
            IEnumerable<AxisValue> axsVal = CalculateQuantityFunction(node.WidgetAggragationFunction, node.StartDate.ToDateTimeOffsetInSeconds(), resolutionInSeconds, data);
            response.Add(axsVal);
            if (node.Markers != null)
            {
                //var resolutionInSeconds = GroupByCalcFunction.ParseAndConvertToSecond(node.WidgetResolution);
                foreach (var markQnt in node.Markers)
                {
                    if (markQnt.Operation.HasValue)
                    {
                        axsVal = CalculateQuantityFunction(markQnt.Operation.ToString(), node.StartDate.ToDateTimeOffsetInSeconds(), resolutionInSeconds, data);
                        response.Add(axsVal);
                    }
                }
            }          
    ...
    private IEnumerable<GraphParametersComponentDtoV3> GetAxisesForMultiParameter(CustomParameterNodeCalculator node)
    {
        ...
        foreach (var finalMatrix in node.FinalAggregationMatrixes)
        {
            ...
            List<IEnumerable<AxisValue>> axis = new List<IEnumerable<AxisValue>>();
            var resolutionInSeconds = node.WidgetResolutionAutoOrInSeconds;
            IEnumerable<IEnumerable<BasicValue>> data;
            GroupByFunctionInput groupByOperation;
            if (node.IsGroupByTime)
            ...
            IEnumerable<AxisValue> axesCollection = CalculateQuantityFunction(node.WidgetAggragationFunction, node.StartDate.ToDateTimeOffsetInSeconds(), resolutionInSeconds, data);
            axis.Add(axesCollection);

            if (node.Markers != null)
            {
                foreach (var item in node.Markers)
                {
                    axesCollection = CalculateQuantityFunction(item.Operation.ToString(), node.StartDate.ToDateTimeOffsetInSeconds(), resolutionInSeconds, data);
                    axis.Add(axesCollection);
                }
            }           
            ...
            foreach (var axisCollection in axis)
            {
                if (node.IsGroupByTime)
                {
                    if (axisCollection.Any())
                    {
                        ...
                        {
                            var graph = new GraphParametersComponentDtoV3(node.CustomParameterName, node.Feeders, node.CustomParameterType.ToString(), finalMatrix.DataUnitType, [], [item], baseParameterAdditionalInfos);
                            responses.Add(graph);
                        }
                    }
                    else
                    {
                        foreach (var item in node.BarTimeRangeList)
                        {
                            var graph = new GraphParametersComponentDtoV3(node.CustomParameterName, node.Feeders, node.CustomParameterType.ToString(), finalMatrix.DataUnitType, [], axisCollection, baseParameterAdditionalInfos);
                            responses.Add(graph);
                        }
                    }
                }
    ...
    public IEnumerable<GraphParametersComponentDtoV3> PrepareResponseForSPMCAndMPSC(CustomParameterNodeCalculator node, IEnumerable<IEnumerable<AxisValue>> axises)
    {
        var baseParameterAdditionalInfos = new List<MissingBaseParameterInfo>();
        ...
        foreach (var axisCollection in axises)
        {
            if (node.IsGroupByTime)
            {
                if (axisCollection.Count() > 0)
                {
                    foreach (var item in axisCollection)
                    {
                        var result = new GraphParametersComponentDtoV3(node.CustomParameterName, feeders, node.CustomParameterType.ToString(), dataUnitType, [], [item], baseParameterAdditionalInfos);
                        graphs.Add(result);
                    }
                }
                else
                {
                    foreach (var item in node.BarTimeRangeList)
                    {
                        var result = new GraphParametersComponentDtoV3(node.CustomParameterName, feeders, node.CustomParameterType.ToString(), dataUnitType, [], axisCollection, baseParameterAdditionalInfos);
                        graphs.Add(result);
                    }
                }
            }
    ...
    public IEnumerable<BasicValue> CalculatedInnerAlignment(CustomParameterNodeCalculator node, BaseParameterComponent item)
    {
        ...
        {
            var points = item.Axis.DataTimeStamps.Select(x => new BasicValue(x.Point, x.DateTime, x.DataValueStatus.ToPqbiDataValueStatus())).ToArray();
