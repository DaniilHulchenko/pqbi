...
public class CustomParameterNodeCalculator
{
        ...
        )
    {
        ...
        StartDate = startDate;
        EndDate = endDate;
        BarTimeRangeList = barTimeRangeList;
        if (barTimeRangeList != null)
            IsGroupByTime = true;
        IsHideMsrPointName = isHideMsrPointName;
    ...
    public void ClearMsrPrmCollection()
    {
        _baseParameterComponents.Clear();
    }

    public ParameterMatrix CalculatedInnerAlignment(IEnumerable<BaseParameterComponent> baseParameterComponents, AdvancedSettings? advancedSettings = null)
    {
        ...
        foreach (var childCalculator in Children)
        {
            ...
            foreach (var item in childCalculator.FinalAggregationMatrixes)
            {
                ...
                string customParameterName = $"{childCalculator.CustomParameterName}_{matrixNum}";
                matrix.AddSeries(customParameterName, calculated, PQZStatus.OK);
                matrixNum++;
            }
        }
    ...
    public void CalculatedParameterOuterMatrixAndAggregation(ParameterMatrix prmMatrix, bool isIgnoreAligningFunction)
    {
        using (var mainLogger = PqbiStopwatch.Anchor($"{nameof(CalculatedParameterOuterMatrixAndAggregation)}"))
        {
            ...
            if (ParameterMatrixes.Count > 0)
            {
                ...
                {
                    prmMatrix.CalculateAndSetOutterAggregation(aggregationFunction);
                    //foreach (var parameterMatrix in prmMatrix)
                    //{
                ...
                {
                    prmMatrix.CalculateAndSetOutterAggregationOnSingleCollection(aggregationFunction);
                }
            }
            //else
            //{
