...
public interface IFeederChanelTreBuilder
{
   Task<StaticTreeNode> GetLogicalOrChannelTreeAsync(params string[] parameters);
}

public class FeederChanelTreBuilder : IFeederChanelTreBuilder
{
    ...
    public async Task<StaticTreeNode> GetLogicalOrChannelTreeAsync(string[] parameters)
    {
        var tree = new StaticTreeNode { Value = StaticTreeNode.RootLabel };

        var logicalTree = new StaticTreeNode { Value = StaticTreeNode.LogicalLabel };
        var channelTree = new StaticTreeNode { Value = StaticTreeNode.ChannelLabel };
    ...
    private void CreateFeederTreeFromParameter(string parameterName, StaticTreeNode tree)
    {
        ...
        if (parameterName.Contains(Feeder_Name))
        {
            ...
            var @params = parameterName
                    .Split('_')
                    .Select((value, index) => IsFeederTreeMapper(isMulti, value, index, paramInfo))
                    .ToList();
            ...
            foreach (var param in @params)
            {
                ...
                if (childNode == null)
                {
                    childNode = new StaticTreeNode { Value = param.value, Description = param.description };
    ...
    private (string value, string description) IsFeederTreeMapper(bool isMulti, string value, int index, ParameterPartDto2 paramInfo)
    {
    ...
    private (string value, string description) IsChannelTreeMapper(bool isMulti, string value, int index, ParameterPartDto2 paramInfo)
    {
        var result = (value, value);
    ...
    private void CreateChannelTreeFromParameter(string parameterName, StaticTreeNode tree)
    {
        if (string.IsNullOrEmpty(parameterName))
        {
            return;
        ...
        if (parameterName.Contains(Channel_Name))
        {
            ...
            var @params = parameterName
                  .Split('_')
                  .Select((value, index) => IsChannelTreeMapper(isMulti, value, index, paramInfo))
                  .ToList();
            var currentNode = tree;
