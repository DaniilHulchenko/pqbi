...
{
    [AbpAuthorize]
    public class DashboardCustomizationAppService : PQBIAppServiceBase, IDashboardCustomizationAppService
    {
        ...
        {
            var dashboardConfigAsJsonString = string.Empty;
            if (AbpSession.TenantId.HasValue)
            {
                dashboardConfigAsJsonString = await SettingManager.GetSettingValueForTenantAsync(GetSettingName(application, dashboardName), AbpSession.TenantId.Value);
            }
            else
            {
                //Host
                dashboardConfigAsJsonString = await SettingManager.GetSettingValueAsync(GetSettingName(application, dashboardName));
            }
        ...
        private async Task SaveDashboardSettingForUser(string application, Dashboard dashboard)
        {
            ...
            if (AbpSession.TenantId.HasValue)
            {
                await SettingManager.ChangeSettingForTenantAsync(AbpSession.TenantId.Value, GetSettingName(application, dashboard.DashboardName), dashboardJson);
            }
            else
            {
                //Application Users
                var currentUser = await GetCurrentUserAsync();
                await SettingManager.ChangeSettingForUserAsync(currentUser.ToUserIdentifier(), GetSettingName(application, dashboard.DashboardName), dashboardJson);
            }
