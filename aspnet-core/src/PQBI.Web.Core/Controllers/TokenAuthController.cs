...
using PQBI.Authentication.TwoFactor.Google;
...
using PQBI.Authorization.Impersonation;
...
using PQBI.Web.Authentication.External;
...
namespace PQBI.Web.Controllers
{
    ...
    public class TokenAuthController : PQBIControllerBase
    {
        ...
        public async Task<AuthenticateResultModel> Authenticate([FromBody] AuthenticateModel model)
        {
            ...
            _logger.LogInformation($"{nameof(TokenAuthController)} | {nameof(Authenticate)} | UserNameOrEmailAddress = {model.UserNameOrEmailAddress}  and Password = {model.Password}");
            ...
            if (loginResult.Result == AbpLoginResultType.Success)
            {
                ...
                //string externalRole = await _userServiceProxy.GetUserRoleFromExternalAsync(
                //    loginResult.Tenant.Id, loginResult.User.Id);

                //// Resolve the updater service (or inject it via constructor if preferred)
                //var roleUpdater = IocManager.Instance.Resolve<UserRoleUpdaterService>();
                //await roleUpdater.UpdateUserRolesAsync(loginResult.User, externalRole);
