...
public class ParameterMatrix : MatrixBase
{
    ...
    {
        if ((sycResolutionInSeconds == resolutionResolutionInSeconds && nominalVal == -1 && !isExcludeFlagged) || points.IsCollectionEmpty())
        {
            return points;
        }
        ...
        if (!string.IsNullOrEmpty(quantityAggregationFunction))
        {
        ...
        {
            calculated = FilterByFlaggedEvents(isExcludeFlagged, points);
        }
    ...
    public static List<BasicValue> FilterByFlaggedEvents(bool isExcludeFlagged, IEnumerable<BasicValue> groupByResponse)
    {
        List<BasicValue> pointsGroupList = null;
        if (!isExcludeFlagged)
        {
            pointsGroupList = groupByResponse.ToList();
        }
        else
        {
            pointsGroupList = new List<BasicValue>();
            foreach (var pointsGroup in groupByResponse)
            {
                if (pointsGroup.DataValueStatus != PqbiDataValueStatus.Flag)
                {
                    pointsGroupList.Add(pointsGroup);
                }
            }
        }
    ...
    public List<BasicValue> Normalize(double nominalVal, List<BasicValue> baseValList)
    {
        ...
        else if (nominalVal != -1)
        {
            ...
            for (int i = 0; i < baseValList.Count; i++)
            {
                ...
                BasicValue newBasicValue = new BasicValue(pointVal, baseValList[i].StartTime, baseValList[i].DataValueStatus);
