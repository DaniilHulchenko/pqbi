...
import { DynamicTreeBuilderComponent } from '@app/shared/common/components/dynamic-tree-builder/dynamic-tree-builder.component';
...
,
    ...
    templateUrl: './cp-components-selector.component.html',
    styleUrl: './cp-components-selector.component.css',
    providers: [
        {
            provide: NG_VALUE_ACCESSOR,
            useExisting: forwardRef(() => CpComponentsSelectorComponent),
            multi: true,
        },
    ],
})
export class CpComponentsSelectorComponent {
    @Input() height: string;
    ...
    writeValue(state: ComponentsState): void {
        if (state) {
            state = this.adjustComponentsState(state);
            this.state = state;
    ...
    private adjustComponentsState(state: ComponentsState) : ComponentsState {
        ...
        if (component.parameterInfos && component.parameterInfos.length > 0) {
            return state;
        }

        let componentKeys = state.components.map(c => c.key);
        let selectedComponents = this.formattedComponentsOptions.filter(c => componentKeys.includes(c.key));
        state.components = selectedComponents;
