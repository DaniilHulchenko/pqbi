import { Component, EventEmitter, forwardRef, input, Input, Output, ViewChild } from '@angular/core';
...
import { DxButtonModule, DxLoadIndicatorModule, DxPopoverModule, DxScrollViewModule } from 'devextreme-angular';
...
@Component({
    ...
    templateUrl: './limited-components-selector.component.html',
    styleUrl: './limited-components-selector.component.css',
    providers: [
        {
            provide: NG_VALUE_ACCESSOR,
            useExisting: forwardRef(() => LimitedComponentsSelectorComponent),
            multi: true,
        },
    ],
})
export class LimitedComponentsSelectorComponent implements ControlValueAccessor {
    @Input() height: string;
    @Input() isFeederSelectionEnabled = false;
    @Input() maxComponentsCount: number;
    @Input() maxFeedersCount: number;
    ...
    state: ComponentsState = new ComponentsState({
        components: null,
        tags: null,
        pickListState: { source: [], target: [] },
    });
    ...
    writeValue(state: ComponentsState): void {
        if (state) {
            ...
            this.state = new ComponentsState({
                components: null,
                tags: null,
                pickListState: { source: [], target: [] },
            });
    ...
    onSelectionChange(event: any[]) {
        ...
        if (this.isFeederSelectionEnabled) {
            ...
            let selectedComponents = componentsAndFeeders?.filter(
        ...
        if (this.maxComponentsCount) {
            let components = this.getDistinctComponents(
                this.selectedItems.filter((component) => component.type !== TreeComponentType.Feeder && component.leaf),
            );
            components = components?.slice(-this.maxComponentsCount);
            const componentKeys = components.map((item) => item.key);

            const parentKeys = components.map((item) => this.getAllParentKeys(item));

            this.selectedItems = this.selectedItems.filter(
                (item) =>
                    componentKeys.includes(item.key) ||
                    componentKeys.includes(item.parentKey) ||
                    parentKeys.flat().includes(item.key),
            );

            if (this.maxFeedersCount) {
                let feeders = this.getDistinctComponents(
                    this.selectedItems.filter((component) => component.type === TreeComponentType.Feeder),
                );
                feeders = feeders?.slice(-this.maxFeedersCount);
                const feederKeys = feeders.map((item) => item.id);
                this.selectedItems = this.selectedItems.filter(
                    (item) => item.type !== TreeComponentType.Feeder || feederKeys.includes(item.id),
                );
            }
        }

        tags = JSON.parse(safeStringify(tags));

        tags.forEach((t) => this.removeRedundantInfoInTag(t));

        this.state.tags = tags;
        this.state.components = this.getDistinctComponents(
            this.selectedItems.filter((component) => component.type !== TreeComponentType.Feeder && component.leaf),
        );
        this.state.feeders = this.getDistinctComponents(
            this.selectedItems.filter((component) => component.type === TreeComponentType.Feeder),
        )?.map((feeder) => {
            return new FeederComponentInfo({
                id: feeder.id,
                name: feeder.label,
                componentId: feeder.parentKey,
                compName: '',
            });
        });
    ...
    private getAllParentKeys(componentNode) {
        if (!componentNode.parent) {
            return [];
        }

        return [componentNode.parent.key, ...this.getAllParentKeys(componentNode.parent)];
    }
}
